...
- **Never** treat an autonomous-mode "go ahead" or "proceed" as ADR
  acceptance. Blanket approvals authorise wave execution, not design
  acceptance — they do not cover this skill.
- **Never** promote in the same turn as `$adr-proposal`. The
  Proposed commit and the Accepted commit must be in separate turns
  so review can actually happen between them. A separate-turn gate
  is the minimal mechanical guard against same-session Proposed→
  Accepted bypass.
