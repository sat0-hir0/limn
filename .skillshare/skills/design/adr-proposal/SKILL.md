...
- Promotion to Accepted happens in a **separate commit** via
  `$adr-acceptance`, after the maintainer has had time to read the
  proposal. Do not chain the two commits in the same change set —
  see `$adr-acceptance` Boundary for the separate-commit guard.
