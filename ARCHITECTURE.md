...
within a folder. Built in Rust + gpui (Zed's GUI framework), it uses Markdown as the
sole storage format.
...
- **Quiet UI** — Nothing extra on screen. iA Writer-style quietness. No handles, hover menus, or toolbars.
- **Keyboard-first** — Vim modal mode is not adopted. Modeless and fast — the Zed approach.
...
- **Alternative 2**: Proactive background suggestions (Copilot-style; tension with simplicity).
